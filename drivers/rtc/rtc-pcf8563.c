				     ...
				     struct device_attribute *attr, char *buf)
{
	...
	if (val & PCF8563_BIT_TEST1)
		size += snprintf(buf, PAGE_SIZE - size, " TEST1");

	if (val & PCF8563_BIT_STOP)
		size += snprintf(buf, PAGE_SIZE - size, " STOP");

	if (val & PCF8563_BIT_TESTC)
		size += snprintf(buf, PAGE_SIZE - size, " TESTC");

	return snprintf(buf, PAGE_SIZE - size, "\n");
}
				     ...
				     struct device_attribute *attr, char *buf)
{
	...
	if (val & PCF8563_BIT_TI_TP)
		size += snprintf(buf, PAGE_SIZE - size, " TI_TP");

	if (val & PCF8563_BIT_AF)
		size += snprintf(buf, PAGE_SIZE - size, " AF");

	if (val & PCF8563_BIT_TF)
		size += snprintf(buf, PAGE_SIZE - size, " TF");

	if (val & PCF8563_BIT_AIE)
		size += snprintf(buf, PAGE_SIZE - size, " AIE");

	if (val & PCF8563_BIT_TIE)
		size += snprintf(buf, PAGE_SIZE - size, " TIE");
